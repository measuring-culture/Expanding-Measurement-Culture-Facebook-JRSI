...
def MantelTest(X, Y, perms=10000, method='pearson', tail='two-tail', ci=None):
  """
  ...
  if ci is not None:
    ci = ( covariances[0] + np.percentile(covariances, ci)) / np.sqrt((X_residuals ** 2).sum() * (Y_residuals ** 2).sum())
    return r, p, z, ci
  else:
    return r, p, z
